...
# def how_many_journalists(arr)
#   return arr.length()
# end

# def find_char_in(char, arr)
#   results = []
#   arr.each do |c|
#     if c.include?(char)
#     results.push(c)
...
# def 00_nbr_of_handles_contain_nbr_in(arr)
#   arr.each do |c|
...
# def perform
#   puts "Il y a #{how_many_journalists($journalists)} journalistes!"
# end
# find_char_in("c", $journalists)
def get_length(arr)
  puts "La longueur de l'array est de #{arr.length}"
end
...
def nbr_of_journalists_with_a_number
  ...
  puts "Il y a #{nbr} journalistes avec un numéro dans leur handle"
end
...
def nbr_that_includes_in(arr, what)
  ...
  puts "Il y a #{nbr} journalistes qui contiennent #{what}"
...
def nbr_that_contain_upcase (arr)
  nbr = 0
  arr.each do |entry|
    if contains_maj(entry)
      nbr +=1
    end
  end
  puts "Il y a #{nbr} journalistes qui ont une majuscule dans leur handle!"
end
def nbr_that_begins_with_uppercase (arr)
  nbr = 0
  arr.each do |entry|
